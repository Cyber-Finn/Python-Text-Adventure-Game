...
def hideInShadows():
    ...
    if choice == "1":
        ...
        print(item)
        #having issues here, because the item is dumped memory, because the import in controller.py is not working - even though it can see the package to import..
        print(f"{magic_pendants.get_interaction_description(item)}") # getattr(controller, attrib name) to get the attribute from that file, can also perform a check to see if the attribute exists like: if hasattr(controller, item.interaction_description)
    elif choice == "2":
        continueHiding()
    else:
        print("Invalid choice. Try again.")
...
def confrontFigure():
    ...
    print("She introduces herself as Erin, a fellow traveler.")

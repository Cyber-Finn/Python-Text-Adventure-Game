...
def dungeon_intro_scene(playerObject: player.Player):
    ...
    if choice == "1":
        ...
        text_colours.printYellowText(["Invalid choice. Try again."])
...
def exitTunnel(playerObject: player.Player): 
    text_colours.printBlueText(["Common sense prevails! You turn and run as fast as you can.", "After what feels like an eon, you find yourself standing in a forest clearing."])
...
def deeperIntoTunnel(playerObject: player.Player):
    ...
    text_colours.printGreenText(["Do you:", "1. Hide in the shadows.", "2. Confront the approaching figure."])
...
def hideInShadows(playerObject: player.Player):
    ...
    text_colours.printGreenText(["Do you pick it up?", "1. Yes", "2. No"])
    ...
    if choice == "1":
        ...
        text_colours.printCyanText([f"{magic_pendants.get_interaction_description(item.name)}"])
        text_colours.resetColorToWhite()
